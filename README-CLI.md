...
| `LLM_ADAPTER_LOG_PRETTY_FILE_MODE` | Pretty request/response file log mode (default `async`) | "sync", "async", or "off" |
| `LLM_ADAPTER_LLM_STREAM_CHUNK_LOGS_ENABLED` | Enable per-chunk streaming info logs (default `0`) | "1" or "0" |
...
| Variable | Purpose | Values |
|----------|---------|--------|
| `LLM_ADAPTER_OUTBOUND_HTTP_KEEPALIVE_ENABLED` | Enable outbound HTTP keep-alive for LLM HTTP calls (default `1`) | "1" or "0" |
