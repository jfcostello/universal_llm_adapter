...
import { assertValidEmbeddingSpec } from '../../../transport/spec-validator.js';
...
export async function handleEmbeddingsRun(
  ctx: HandlerContext,
  ...
  const release = await ctx.embeddingRunLimiter.acquire(abortController.signal);
  ...
  try {
    ...
    const createEmbeddingCoordinator = (ctx.deps as any).createEmbeddingCoordinator as
      | (typeof ctx.deps)['createEmbeddingCoordinator']
      | undefined;
    ...
    const callPromise = ctx.runWithCoordinatorLifecycleFn<EmbeddingCallSpec, any, any, any>({
      spec,
      pluginsPath: ctx.pluginsPath,
      registry: ctx.registry,
      batchId: ctx.batchId,
      closeLoggerAfter: ctx.closeLoggerAfterRequest,
      deps: { ...ctx.deps, createCoordinator: createEmbeddingCoordinator },
