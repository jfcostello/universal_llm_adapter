...
import { createProviderEventLogger } from './provider-event-log.js';
...
export function createGrokRealtimeCompatSessionWithTransport(
  ...
  const providerEventLogger = createProviderEventLogger(sessionId);
  ...
  void (async () => {
    try {
      for await (const evt of transport.events()) {
        ...
        if (readyAlreadySent) {
          providerEventLogger.maybeLog(parsed);
        }
