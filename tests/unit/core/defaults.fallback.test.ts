...
describe('core/defaults loading fallback', () => {
  ...
  test('uses fallback when no JSON file exists (via module reimport with mocked fs)', async () => {
    ...
    expect((defaults as any).outboundHttp.keepAliveEnabled).toBe(true);
    expect((defaults as any).outboundHttp.maxSockets).toBe(256);
    expect((defaults as any).outboundHttp.maxFreeSockets).toBe(32);
    expect((defaults as any).logging.prettyFileLogs.mode).toBe('async');
    expect((defaults as any).logging.llmStream.chunkInfoLogsEnabled).toBe(false);
