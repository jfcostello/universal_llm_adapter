...
describe('core/logging voice + realtime file logs', () => {
  ...
  test('VoiceLogger rotates batch logs when max-bytes is exceeded', async () => {
    await withTempCwd('voice-logger-rotate-bytes', async (cwd) => {
      process.env.LLM_ADAPTER_BATCH_ID = 'batch_1';
      process.env.LLM_ADAPTER_BATCH_DIR = '1';
      ...
      const batchDir = path.join(cwd, 'logs', 'voice', 'batch-batch_1');
      fs.mkdirSync(batchDir, { recursive: true });
      const active = path.join(batchDir, 'voice.log');
      fs.writeFileSync(active, 'x'.repeat(50), 'utf-8');

      const logger = new VoiceLogger(LogLevel.INFO, 'corr-voice');
      logger.info('voice.rotate', { ok: true });
      await logger.close();

      const files = fs.readdirSync(batchDir);
      expect(files).toContain('voice.log');
      const rotated = files.filter(f => f.startsWith('voice-') && f.endsWith('.log'));
      expect(rotated.length).toBeGreaterThan(0);
      expect(fs.readFileSync(path.join(batchDir, rotated[0] as string), 'utf-8')).toContain('x');
      expect(fs.readFileSync(active, 'utf-8')).toContain('voice.rotate');
    });
  });
  ...
  test('VoiceLogger evaluates retention matchers for non-batch log directories', async () => {
    await withTempCwd('voice-logger-retention-matchers', async (cwd) => {
      jest.useFakeTimers().setSystemTime(new Date('2025-10-18T10:00:00.000Z'));

      const dir = path.join(cwd, 'logs', 'voice');
      fs.mkdirSync(dir, { recursive: true });
      fs.writeFileSync(path.join(dir, 'voice-old.log'), 'x', 'utf-8');
      fs.writeFileSync(path.join(dir, 'other.log'), 'y', 'utf-8');
      fs.mkdirSync(path.join(dir, 'voice-dir'), { recursive: true });

      const { module } = await setupLoggingTestHarness({ disableFileLogs: false });
      const { VoiceLogger, LogLevel } = module as any;

      const logger = new VoiceLogger(LogLevel.INFO, 'corr-voice');
      logger.info('voice.retention', { ok: true });
      await logger.close();

      const files = fs.readdirSync(dir);
      expect(files.some(f => f.includes('voice') && f.endsWith('.log'))).toBe(true);
    });
  });

  test('VoiceLogger rotates batch-file logs using baseName when batch dir is disabled', async () => {
    await withTempCwd('voice-logger-rotate-batch-file', async (cwd) => {
      process.env.LLM_ADAPTER_BATCH_ID = 'batch_1';
      process.env.LLM_ADAPTER_VOICE_LOG_MAX_BYTES = '1';
      jest.useFakeTimers().setSystemTime(new Date('2025-10-18T10:00:00.000Z'));
      ...
      const active = path.join(dir, 'voice-batch-batch_1.log');
      fs.writeFileSync(active, 'x'.repeat(50), 'utf-8');

      const logger = new VoiceLogger(LogLevel.INFO, 'corr-voice');
      logger.info('voice.rotate.batch_file', { ok: true });
      await logger.close();

      const files = fs.readdirSync(dir);
      expect(files).toContain('voice-batch-batch_1.log');
      const rotated = files.filter(f => f.startsWith('voice-batch-batch_1-') && f.endsWith('.log'));
      expect(rotated.length).toBeGreaterThan(0);
    });
  });

  test('VoiceLogger byte tracking and flush scheduler are safe under edge conditions', async () => {
    await withTempCwd('voice-logger-internal-guards', async () => {
      const { module } = await setupLoggingTestHarness({ disableFileLogs: false });
      const { VoiceLogger, LogLevel } = module as any;
  ...
  test('VoiceLogger guards against unexpected stat output and disabled file logging', async () => {
    await withTempCwd('voice-logger-current-bytes-guards', async () => {
      ...
      const logger = new VoiceLogger(LogLevel.INFO, 'corr-voice');
      logger.info('voice.current_bytes');
  ...
  test('VoiceLogger max-bytes rotation guard exits early when not in batch mode', async () => {
    await withTempCwd('voice-logger-rotate-guard-no-batch', async () => {
      process.env.LLM_ADAPTER_VOICE_LOG_MAX_BYTES = '1000';
      jest.useFakeTimers().setSystemTime(new Date('2025-10-18T10:00:00.000Z'));
      ...
      const logger = new VoiceLogger(LogLevel.INFO, 'corr-voice');
      logger.info('voice.rotate.guard', { ok: true });
      await expect((logger as any).maybeRotateForMaxBytes(1)).resolves.toBeUndefined();
      await logger.close();
    });
  });

  test('VoiceLogger max-bytes rotation guard is a no-op when below threshold', async () => {
    await withTempCwd('voice-logger-rotate-guard-below-threshold', async () => {
      process.env.LLM_ADAPTER_BATCH_ID = 'batch_1';
      process.env.LLM_ADAPTER_VOICE_LOG_MAX_BYTES = '1000';
      jest.useFakeTimers().setSystemTime(new Date('2025-10-18T10:00:00.000Z'));
