...
async function startHarness(options: {
  config: { path: string; maxMessageBytes: number; idleTimeoutMs: number };
...
describe('server/internal/realtime/ws', () => {
  test('rejects upgrade when path does not match (and exercises parseWsPath branches)', async () => {
    const harness = await startHarness({
      config: { path: '/realtime/ws', maxMessageBytes: 1024 * 1024, idleTimeoutMs: 0 },
  ...
  test('closes idle connections with ws_idle_timeout', async () => {
    const harness = await startHarness({
      config: { path: '/realtime/ws', maxMessageBytes: 1024 * 1024, idleTimeoutMs: 25 },
      createSession: jest.fn()
    });
  ...
  test('fails on invalid JSON and oversized messages', async () => {
    const harness = await startHarness({
      config: { path: '/realtime/ws', maxMessageBytes: 64, idleTimeoutMs: 0 },
      createSession: jest.fn()
    });
