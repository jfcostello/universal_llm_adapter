...
describe('utils/server auth config resolution', () => {
  ...
  test('createServer throws when auth.enabled is provided', async () => {
    jest.resetModules();
    const { createServer } = await import('@/modules/server/index.ts');
    await expect(createServer({ auth: { enabled: true } as any } as any)).rejects.toThrow(
      'auth.enabled is no longer supported; use auth.mode'
    );
  });

  test('createServer throws when allowApiKeyHeader is provided', async () => {
    jest.resetModules();
    const { createServer } = await import('@/modules/server/index.ts');
    await expect(
      createServer({ auth: { mode: 'apiKey', allowApiKeyHeader: true, keys: [{ id: 'k1', token: 'k1' }] } as any } as any)
    ).rejects.toThrow('allowApiKeyHeader is no longer supported; use allowHeader');
  });
  ...
  test('createServer supports jwt auth mode passthrough', async () => {
    jest.resetModules();
    ...
    const running = await createServer({
      registry: registry as any,
      auth: { mode: 'jwt', jwks: { keys: [] } } as any,
      deps: {
        createRegistry: jest.fn().mockResolvedValue(registry),
        createCoordinator: jest.fn(),
        closeLogger: jest.fn().mockResolvedValue(undefined)
    ...
    expect(capturedAuth).toMatchObject({ mode: 'jwt' });
    await running.close();
  });
