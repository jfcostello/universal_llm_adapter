...
describe('realtime-compat/gemini — event mapper', () => {
  ...
  test('maps usageMetadata', () => {
    ...
    expect(events[0]).toEqual({
      ...
      inputTokens: 1,
      outputTokens: 2,
      metadata: { totalTokenCount: 3, cachedContentTokenCount: undefined }
    });
  });
  ...
  test('usageMetadata handles missing/non-numeric fields', () => {
    ...
    expect(events[0]).toEqual({
      ...
      metadata: { totalTokenCount: undefined, cachedContentTokenCount: 9 }
    });
  });
  ...
  test('maps transcripts, audio chunks, text, and turn completion', () => {
    ...
    expect(first.some(e => e.type === 'assistant_text.delta')).toBe(true);
    ...
    expect(complete.some(e => e.type === 'assistant_text.final')).toBe(true);
  });
