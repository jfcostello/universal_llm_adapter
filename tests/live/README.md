...
The live runner loads `./.env` and hard-fails when required env vars are missing (see `tests/live/required-env.ts`).
...
ANTHROPIC_API_KEY=...
OPENAI_API_KEY=...
OPENAI_ASSISTANTS_ASSISTANT_ID=asst_...
GEMINI_API_KEY=...

# Azure OpenAI Assistants (when selected)
