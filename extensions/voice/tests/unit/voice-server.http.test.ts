...
describe('extensions/voice: server http handlers', () => {
  ...
  test('/voice/webhook logs provider plugin loader warnings when logging is available', async () => {
    ...
    try {
      await fs.mkdir(path.join(tmp, 'providers'), { recursive: true });
      await fs.writeFile(path.join(tmp, 'providers', 'bad.json'), JSON.stringify('not-an-object'), 'utf-8');
      await fs.writeFile(path.join(tmp, 'providers', 'test.json'), JSON.stringify({ id: 'test', kind: 'test' }, null, 2), 'utf-8');
      await fs.mkdir(path.join(tmp, 'compat', 'test'), { recursive: true });
      await fs.writeFile(
        path.join(tmp, 'compat', 'test', 'index.js'),
        [
  ...
  test('/voice/webhook (GET) passes the full URL (including query string) to signature validation', async () => {
    ...
    test('/voice/webhook (POST) treats empty json body as {} and supports webhook-side events.emit', async () => {
      process.env.LLM_ADAPTER_VOICE_WS_TOKEN_SECRET = 'secret';
  ...
  test('POST /voice/calls accepts providerConfig and persists it to the call config', async () => {
    process.env.LLM_ADAPTER_VOICE_WS_TOKEN_SECRET = 'secret';
    ...
    const callConfig = await store.getConfig(String(response.callConfigId));
    expect(callConfig?.providerConfig).toEqual({ foo: { bar: 1 } });
  });

  test('POST /voice/calls rejects providerConfig when it is not an object', async () => {
    process.env.LLM_ADAPTER_VOICE_WS_TOKEN_SECRET = 'secret';
