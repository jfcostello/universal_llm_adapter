...
import { createVoiceMediaWsBridge } from '@/extensions/voice/modules/media-bridge/index.ts';
import {
  connectedMessage,
  TEST_MEDIA_PROTOCOL_ADAPTER,
  createTestMediaBridge,
  dtmfMessage,
  markMessage,
  mediaMessage,
  startMessage,
  stopMessage
} from '../helpers/test-media-protocol.ts';
...
describe('extensions/voice/modules/media-bridge: bridge coverage', () => {
  ...
  test('streamSid mismatch closes', async () => {
    ...
    await task;
    expect(onError).toHaveBeenCalledWith(expect.objectContaining({ code: 'stream_id_mismatch' }));
    jest.useRealTimers();
  });
  ...
  test('invalid JSON message closes with invalid_message', async () => {
    jest.useFakeTimers().setSystemTime(new Date('2025-01-01T00:00:00.000Z'));
    const secret = 'secret';
    const token = makeToken(secret);
    const onError = jest.fn();
  ...
  test('session ready not first closes with unexpected_session_event', async () => {
    jest.useFakeTimers().setSystemTime(new Date('2025-01-01T00:00:00.000Z'));
    const secret = 'secret';
    const token = makeToken(secret);
    const onError = jest.fn();
    ...
    ws.emitMessage(startMessage({ customParameters: { from: 'x', to: 'y' } }));
    await task;
    expect(onError).toHaveBeenCalledWith(expect.objectContaining({ code: 'unexpected_session_event' }));
    jest.useRealTimers();
  });

  test('session ends before ready closes with session_ended_before_ready', async () => {
    jest.useFakeTimers().setSystemTime(new Date('2025-01-01T00:00:00.000Z'));
    const secret = 'secret';
    const token = makeToken(secret);
    const onError = jest.fn();
    ...
    expect(onError).toHaveBeenCalledWith(expect.objectContaining({ code: 'session_ended_before_ready' }));
    jest.useRealTimers();
  });
  ...
  test('adapter can return pre-serialized string messages', async () => {
    jest.useFakeTimers().setSystemTime(new Date('2025-01-01T00:00:00.000Z'));
    ...
    const adapter = {
      ...TEST_MEDIA_PROTOCOL_ADAPTER,
      buildClearMessage: ({ streamId }: { streamId: string }) => JSON.stringify({ event: 'clear', streamSid: streamId })
    };
    ...
    const ws = new MockWebSocket();
    const task = bridge.handleConnection(ws as any, {} as any);
    ws.emitMessage(startMessage({ customParameters: { from: 'x', to: 'y' } }));
    await flush();
